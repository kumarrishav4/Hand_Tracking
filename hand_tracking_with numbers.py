...
def lines_and_box(image, points):
    ...
    for i, point in enumerate(points):
        ...
        cv2.putText(image, str(i), (int(x) + 10, int(y) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    for i in range(len(palm_box) - 1):
        ...
        cv2.putText(image, str(i), (int(palm_box[i][0]) + 10, int(palm_box[i][1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    cv2.line(image, (int(palm_box[-1][0]), int(palm_box[-1][1])), (int(palm_box[0][0]), int(palm_box[0][1])), (0, 255, 0), 2)
    cv2.putText(image, str(len(palm_box)-1), (int(palm_box[-1][0]) + 10, int(palm_box[-1][1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
def capture_and_track():
    ...
    cv2.namedWindow("Hand Tracking with Numbers", cv2.WINDOW_NORMAL)
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Hand Tracking with Box and Numbers", frame)
